...
alpha2 = 2/3; beta2 = 4/3; mygam2 = 1;delta2 = 1;
...
%   alpha1=2/3;beta1=4/3;mygam1=1;delta1=1;
%   alpha2=2/3;beta2=4/3;mygam2=0.1;delta2=1/4;

mydiff = [0 0.0001 0.001 0.01]';
...
fprey1=@(t,prey1,pred1,prey2) alpha1*prey1-beta1*prey1.*pred1-mydiff.*(prey1-prey2);
fpred1=@(t,prey1,pred1) -mygam1*pred1+delta1*prey1.*pred1;
fprey2=@(t,prey2,pred2,prey1) alpha2*prey2-beta2*prey2.*pred2-mydiff.*(prey2-prey1);
fpred2=@(t,prey2,pred2) -mygam2*pred2+delta2*prey2.*pred2;
...
prey1 = 2*ones(4,1); pred1 = 0.1*ones(4,1); t=0;
...
%   t = t+dt;
  % Heun time stepping as in notes
    preytilde1 = prey1+dt*fprey1(t,prey1,pred1,prey2);
    predtilde1 = pred1+dt*fpred1(t,prey1,pred1);
    preytilde2 = prey2+dt*fprey2(t,prey2,pred2,prey1);
    predtilde2 = pred2+dt*fpred2(t,prey2,pred2);
    ttilde = t+dt;
    prey1 = prey1+0.5*dt*(fprey1(t,prey1,pred1,prey2)+fprey1(ttilde,preytilde1,predtilde1,preytilde2));
    pred1 = pred1+0.5*dt*(fpred1(t,prey1,pred1)+fpred1(ttilde,preytilde1,predtilde1));
    prey2 = prey2+0.5*dt*(fprey2(t,prey2,pred2,prey1)+fprey2(ttilde,preytilde2,predtilde2,preytilde1));
    pred2 = pred2+0.5*dt*(fpred2(t,prey2,pred2)+fpred2(ttilde,preytilde2,predtilde2));
    t = ttilde;    
 end
 % store
...
clf
betterplots
plot(ts(lhpt:end),preys1(lhpt:end,1)-preys2(lhpt:end,1),'r')
hold on
plot(ts(lhpt:end),preys1(lhpt:end,2)-preys2(lhpt:end,2),'g')
plot(ts(lhpt:end),preys1(lhpt:end,3)-preys2(lhpt:end,3),'m')
plot(ts(lhpt:end),preys1(lhpt:end,4)-preys2(lhpt:end,4),'c')
grid on
ylabel('prey difference')
xlabel('t')